...
def weighted_F1(TP, FP, TN, FN, num_negative, num_positive=41599):
    ...
    positive_F1 = (2 * positive_precision * positive_recall) / (
        positive_precision + positive_recall
    )
    negative_precision = TN / (TN + FN)
    negative_recall = TN / (TN + FP)
    negative_F1 = (2 * negative_precision * negative_recall) / (
        negative_precision + negative_recall
    )
    weighted_F1 = (((num_positive) / (num_positive + num_negative)) * positive_F1) + (
        ((num_negative) / (num_positive + num_negative)) * negative_F1
    )
    return weighted_F1
...
def load_data(data, is_charge_balanced, max_atoms=5, max_coefficient=100000):
    ...
    for i in range(len(data)):
        try:
            mg.core.composition.Composition(data[i, 1])
        except:
...
def get_batch_val(neg_positive_ratio):
    ...
    noTr_positives + (noTr_negatives)  # total size of train set
...
for i in range(num_steps):
    ...
    if i % 1000 == 0:
        ...
        np.savetxt(
            "performance_matrix_TL_v3_25M_" + model_name_params + ".txt",
            performance_array,
            fmt="%s",
        )
    if i % 1000 == 0:
